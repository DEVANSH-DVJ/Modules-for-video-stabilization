...
pwcnet_dir = os.path.dirname(os.path.realpath(__file__))

try:
    from .correlation import correlation  # the custom cost volume layer
except Exception:
    sys.path.insert(0, os.path.join(pwcnet_dir, 'correlation'))
    ...
    def __init__(self):
        ...
        class Decoder(torch.nn.Module):
            def __init__(self, intLevel):
                ...
                self.netSix = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels=intCurrent + 128 + 128 + 96 + 64 +
                                    32, out_channels=2, kernel_size=3, stride=1, padding=1)
                )
            # end
        ...
        self.load_state_dict({strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.hub.load_state_dict_from_url(
            url='http://content.sniklaus.com/github/pytorch-pwc/network-' + arguments_strModel + '.pytorch').items()})
    # end
...
def warp(I2, flow):
    ...
    for i in range(flow.shape[0]):
        for j in range(flow.shape[1]):
            x = j + round(float(flow[i, j, 0]))
            y = i + round(float(flow[i, j, 1]))
            if x >= 0 and x < flow.shape[1] and y >= 0 and y < flow.shape[0]:
                warped[i, j, :] = I2[y, x, :]
    return warped
...
def idct_2d(w):
    global dct_basis, top
    res = torch.zeros_like(dct_basis[0, :, :])
    for i in range(top**2):
        res += w[i // top, i % top] * dct_basis[i, :, :]
...
if __name__ == '__main__':
    ...
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(
        img2.transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))

    tenOutput = estimate(tenOne, tenTwo)
    tenOutput = torch.transpose(tenOutput, 0, 2)
    ...
    tenOutput_4 = np.zeros_like(tenOutput)
    x_4 = np.zeros_like(x)
    x_4[:4, :4] = x[:4, :4]
    tenOutput_4[:, :, 0] = idct2(x_4)
    y_4 = np.zeros_like(y)
    y_4[:4, :4] = y[:4, :4]
